...
import { Tournament, Participant } from '../api/types';
...
const TournamentBracket: React.FC<TournamentBracketProps> = ({
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        const { data, errors } = await client.models.Tournament.get({
          id: tournamentId,
        });
  ...
  if (loading || loadingRounds || loadingMatches) {
    return (
      <div className="text-center py-8">Loading tournament bracket...</div>
    );
  }
      ...
      {rounds.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-primary-text opacity-70">
            Create rounds to start building your bracket!
          </p>
        </div>
      ) : (
                ...
                <div className="space-y-6">
                  {matches
                    ...
                    .map((match) => (
                      <div
                        key={match.id}
                        className="match bg-slate-800 p-4 rounded-lg shadow"
                      >
                        <div
                          className={`participant p-3 rounded mb-2 flex justify-between items-center ${match.winnerId === match.participant1Id ? 'bg-green-900/30 border border-green-700' : 'bg-slate-700'}`}
                        >
                          <span>
                            {participants.find(
                        ...
                        <div className="text-center text-xs text-primary-text opacity-70 my-1">
                          vs
                        </div>

                        <div
                          className={`participant p-3 rounded mb-2 flex justify-between items-center ${match.winnerId === match.participant2Id ? 'bg-green-900/30 border border-green-700' : 'bg-slate-700'}`}
                        >
                          <span>
                            {participants.find(
