...
const UpdateMatchForm: React.FC<UpdateMatchFormProps> = ({ match, onSuccess }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      [name]: name === 'score1' || name === 'score2' ? parseInt(value, 10) : value 
    }));
  };
  ...
  return (
    <div className="card max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          <label htmlFor="score1" className="block mb-2">Score 1</label>
          <input
            type="number"
            id="score1"
            name="score1"
        ...
        <div className="mb-4">
          <label htmlFor="score2" className="block mb-2">Score 2</label>
          <input
            type="number"
            id="score2"
            name="score2"
        ...
        <div className="mb-4">
          <label htmlFor="winnerId" className="block mb-2">Winner</label>
          <select
            id="winnerId"
            name="winnerId"
          ...
          >
            ...
            {formData.participant1Id && (
              <option value={formData.participant1Id}>
                {participants.find(p => p.id === formData.participant1Id)?.name || 'Participant 1'}
              </option>
            )}
            {formData.participant2Id && (
              <option value={formData.participant2Id}>
                {participants.find(p => p.id === formData.participant2Id)?.name || 'Participant 2'}
              </option>
            )}
          </select>
        </div>
        ...
        <div className="mb-6">
          <label htmlFor="songPerformed" className="block mb-2">Song</label>
          <input
            type="text"
            id="songPerformed"
            name="songPerformed"
