...
export const seedDatabase = async (client: any) => {
  ...
  const { data: existingTournaments } = await client.models.Tournament.list({
    limit: 1,
  });
  ...
  for (const tournament of mockTournaments) {
    const { data: newTournament } =
      await client.models.Tournament.create(tournament);
  ...
  for (const participant of mockParticipants) {
    const { data: newParticipant } =
      await client.models.Participant.create(participant);
